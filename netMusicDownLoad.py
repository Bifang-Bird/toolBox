...
class Song():
    ...
    def __init__(self, song_id, song_name, song_num, song_arname,song_url=None):
        self.song_id = song_id
        self.song_name = song_name
        self.song_num = song_num
        self.song_arname = song_arname
...
class Crawler():
    ...
    def search_song(self, song_name, song_num, quiet=True, limit=9):
        ...
        if result['result']['songCount'] <= 0:
            ...
            if quiet:
                ...
                ars = songs[0]['ar']
                arname = ars[0]['name']
                song = Song(song_id=song_id, song_name=song_name, song_num=song_num,song_arname = arname)
                return song
    ...
    def get_song_by_url(self, song_url, song_name, song_num,song_arname, folder):
        """
        下载歌曲到本地
        ...
        if not os.path.exists(folder):
            os.makedirs(folder)
        fpath = os.path.join(folder, str(song_num) + '_' + song_name+'_'+song_arname + '.mp3')
...
class Netease():
    ...
    def download_song_by_search(self, song_name, song_num):
        ...
        # 如果找到了音乐, 则下载
        if song != None:
            self.download_song_by_id(song.song_id, song.song_name, song.song_num,song.song_arname, self.folder)

    def download_song_by_id(self, song_id, song_name, song_num, song_arname,folder='.'):
        """
        通过歌曲的ID下载
        :params song_id: 歌曲ID
        ...
        try:
            ...
            song_name = song_name.replace('.', '')
            self.crawler.get_song_by_url(url, song_name, song_num,song_arname, folder)
